...
if ($database->isInstalled()) {
    try {
        ...
        if (!$ultima_esecuzione) {
            $cron_status = 0;
            $cron_description = tr('Il cron non è stato configurato correttamente. Sembra che il cron di Safix non sia in esecuzione.<br><br>È necessario configurare il cron di sistema in modo che esegua periodicamente il file cron.php di Safix con il seguente comando:<br><br><code>php _DOCUMENT_ROOT_\cron.php</code><br><br>La frequenza suggerita è di 10 minuti, ma può essere adattata alle tue esigenze. Se invii molte newsletter, per esempio, è consigliato inserire 1 minuto come frequenza.', [
            ...
            if ($ore_trascorse > 1) {
                ...
                $cron_description = tr('Sembra che il cron di Safix non sia in esecuzione (ultima esecuzione il _DATA_).<br><br>È necessario configurare il cron di sistema in modo che esegua periodicamente il file cron.php di Safix con il seguente comando:<br><br><code>php _DOCUMENT_ROOT_\cron.php</code><br><br>La frequenza suggerita è di 10 minuti, ma può essere adattata alle tue esigenze. Se invii molte newsletter, per esempio, è consigliato inserire 1 minuto come frequenza.', [
